...
bool DemoInitialize(bbe::RenderParams &params)
{
   ...
   if(result == false)
      {
         printf("Error loading decal.tga\n");
         return false;
      }
...
void DemoRender()
{
   ...
   g_Render.Enable(BB_ALPHA_TESTING);
   g_Render.SetAlphaFunc(BB_ALPHA_GREATER, 0.5f);

   g_Render.Render(BB_RENDER_MODEL_DATA_PARAMS(g_model));

   g_Render.Disable(BB_ALPHA_TESTING);

/*
   Specular Mapping Demo - Chapter 5
...
bbe::ParameterHandle g_camPos;
bbe::ParameterHandle g_shininess;
...
bool DemoInitialize(bbe::RenderParams &params)
{
   ...
   if(CreateOBJMesh("resources/TexturedBox.obj", &g_model) == false)
      return false;
   ...
   if(!g_Render.LoadTexFromFile("resources/decal.tga", BB_TEX2D_TYPE,
      &g_tex))
      {
         return false;
      }
   ...
   g_Render.SetupShaderParameter("camPos", g_shader, &g_camPos);
   g_Render.SetupShaderParameter("shininess", g_shader, &g_shininess);
...
void DemoRender()
{
   ...
   g_Render.SetView(0, 0, 6, 0, 0, 0, 0, 1, 0);
   g_Render.RotateMatrix((float)g_xRot, 0, 1, 0);
   g_Render.RotateMatrix((float)g_yRot, 1, 0, 0);

   g_Render.ApplyShader(g_shader);

   g_Render.SetShaderParameter4f(g_lightPos, 0, 0, 50, 1);
   g_Render.SetShaderParameter4f(g_lightCol, 1, 1, 1, 1);
   g_Render.SetShaderParameter4f(g_camPos, 0, 0, 6, 0);
   g_Render.SetShaderParameter1f(g_shininess, 120);
